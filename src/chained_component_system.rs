...
pub fn generate_chunkiters(
    ...
    for system_sig in system_signatures {
        ...
        for x in system_sig.1.iter(){
            component_tp.push(component_labels.iter().find(|y| {
                y.0.cmp(x).is_eq()
            }).unwrap().1.clone());
        } 

        output.extend(build_chunk_iter_struct(&chunk_iter_name, &system_sig.1,&component_tp));


        for ecs_soa in ecs_soas.iter(){

            if system_sig.1.iter().all( |x| {
                ecs_soa.soa_fields.iter().find(|y| y.0.cmp(x).is_eq()).is_some()
            }) {

                output.extend(build_into_iter(&chunk_iter_name, &ecs_soa.ecs_field.1,&system_sig.1,&component_tp))
...
fn build_into_iter(chunk_iter_name: &Ident, soa_name: &Ident, field_names: &Vec<Ident>, field_types: &Vec<Ident>) -> TokenStream{
    ...
    let f_name = quote::format_ident!("get_{}",chunk_iter_name.to_string().to_lowercase());

    quote!{
        impl #soa_name {

            pub fn #f_name(&mut self) -> #chunk_iter_name {
                #chunk_iter_name {
                    index: 0,
...
fn build_chunk_iter_struct(chunk_iter_name: &Ident, field_names: &Vec<Ident>, field_types: &Vec<Ident>) -> TokenStream{
